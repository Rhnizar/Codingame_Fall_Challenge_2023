...
void    Avoid_Monster(My_drone drone, int Drone_NextX, int Drone_NextY)
{
    ...
    for(size_t j=0; j<drone.Monsters.size(); j++)
    {
        ...
        {
            vector<Point> Dvp = divideLine(Point(drone.drone_x, drone.drone_y), Point(EstablishedPoints[i].x, EstablishedPoints[i].y), 15);
            vector<Point> Mvp = divideLine(Point(Xmonster, Ymonster), Point(Next_Xmonster, Next_Ymonster), 15);

            int check = 0;
            double di = distance(EstablishedPoints[i].x, EstablishedPoints[i].y, Next_Xmonster, Next_Ymonster);
            double di2 = distance(EstablishedPoints[i].x, EstablishedPoints[i].y, Xmonster, Ymonster);
            double disDrone = 999999999;
            if(drone.Monsters[0].creature_id == 0)
            {
                disDrone = distance(EstablishedPoints[i].x, EstablishedPoints[i].y, drone.Monsters[0].creature_x, drone.Monsters[0].creature_y);
                // cerr << "yeeeeeeeeees\n";
            }
            for(size_t r=0; r<Dvp.size(); r++)
            {
                if(distance(Dvp[r].x, Dvp[r].y, Mvp[r].x, Mvp[r].y) <= 510 || di <= 501 || di2 <= 501 || disDrone <= 600 ||
...
void    SepareteDrone(My_drone drone, vector<int> My_drone_scan, vector<Fish> fish, Point &Target, int GlobalScan, map<int, int> RemainingFish)
{
    ...
    // }
    // cerr << "RemainingFish =  " << RemainingFish.size() << endl;
    if(size1 > 0 || GlobalScan == 12 || RadarType2.size() == 0)
    {
    ...
    else if(size1 == 0 && drone.drone_y >= 6200)
    {
        // cerr << "dkhal hna 2\n";
        // cerr << "here\n";
...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < my_drone_count; i++)
        {
            if(my_drone[i].drone_y >= 5600 && my_drone[i].drone_y <= 8000)
                my_drone[i].light = 1;
            if(Turn % 4  == 0 && my_drone[i].drone_y > 2000)
                my_drone[i].light = 1;
            if(my_drone[i].drone_y >= 800 && my_drone[i].drone_y <= 1500)
                my_drone[i].light = 1;
            if(i == 0)
...
// 1320941081848129500. moxkil 9bl lghda 


// 1320941081848129500. dokoko monster

// 8608207385611801000 Monster

// 4063779579303408600.  with boss 

// dyal monster lcase dyal maxayfoxe o kandrob fih 
// 1926786006061197300 
// -5525679507461600000
