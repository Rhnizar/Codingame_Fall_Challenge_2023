...
void    SepareteDrone(My_drone drone, vector<int> My_drone_scan, vector<Fish> fish, Point Target, int GlobalScan)
{
    ...
    cerr << "global scan  " << GlobalScan << endl;
    cerr << "SizeScanType2  " << SizeScanType2 << endl;
    cerr << "RadarType2.size  " << RadarType2.size() << endl;
    if(SizeScanType2 > 0 || GlobalScan == 12 || RadarType2.size() == 0)
    {
        Target.y = 500;
        Target.x = drone.drone_x;
        drone.light = 1;
        if(RadarType1.size() - 1 > 0 || (RadarType2.size() == 0 && RadarType1.size() != 0))
        {
    ...
    // else if((RadarType2.size() == 0 && drone.drone_y >= Target.y) || SizeScanType1 <= 1)
    // {
    //     Target.y = 3500;
    //     rad = Calculate_Numbers_Of_Radar(drone, RadarType1, fish);
...
void MoveDrone(My_drone drone, int &Check, vector<int> My_drone_scan, vector<Visible_creature> Visible_creature, int sep, vector<Fish> fish, int &TmpSize, vector<int> foe_drone_scan, int GlobalScan)
{
    ...
    else
    {
        // int C = 0;
        // rad = Calculate_Numbers_Of_Radar(drone.RadarDrone, fish, 1);
        // if(RadarType2.size() == 0 && drone.drone_y >= Target2.y)
        //     Target2.y = 3500;
        // for(size_t i=0; i<My_drone_scan.size(); i++)
        // {
        //     if(check_id(foe_drone_scan, My_drone_scan[i]) == -1)
