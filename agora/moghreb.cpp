...
#pragma GCC target("avx2,abm,fma,bmi,bmi2,popcnt,lzcnt")
    ...
    for(size_t j=0; j<drone.Monsters.size(); j++)
    {
        ...
        if(EstablishedPoints.size() == 0)
        {
            ...
            for(size_t r=0; r<Dvp2.size(); r++)
            {
                ...
                for(size_t i=0; i<OldMonster.size(); i++)
                {
                    if(distance(Dvp2[r].x, Dvp2[r].y, OldMonster[i].x, OldMonster[i].y) <= 501 || distance(Dvp2[r].x, Dvp2[r].y, OldMonster[i].NextX, OldMonster[i].NextY) <= 501)
                        CheckOldMonster = 1;
                }
...
string Calculate_Numbers_Of_Radar(My_drone drone, vector<Radar> RaV, vector<Fish> fish)
{
    ...
    {
        if(CheckType(RaV[i].creature_id, fish) == 1)
...
Point Target(2500, 6000);
Point Target2(7500, 6000);
...
void    SepareteDrone(My_drone drone, vector<int> My_drone_scan, vector<Fish> fish, Point Target, int GlobalScan)
{
    ...
    {
        if(CheckType(My_drone_scan[i], fish) == 1)
            size1++;
    }
...
int main()
{
    ...
    cerr << "count Monster = " << Monsters.size() << endl;
    int Turn = 0;
    vector<int> My_drone1_scan;
	vector<int> My_drone2_scan;
    vector<int> foe_drone_scan;

    vector <Visible_creature> OldMonstersDrone1;
    vector <Visible_creature> OldMonstersDrone2;
    // game loop
    while (1)
    {
        ...
        for (int i = 0; i < foe_drone_count; i++)
        {
            ...
            int emergency;
            int battery;
            cin >> drone_id >> foe_drone_x >> foe_drone_y >> emergency >> battery; cin.ignore();
            foe_Point.push_back(Point(foe_drone_x, foe_drone_y));
        }
        ...
        for (int i = 0; i < visible_creature_count; i++)
        {
            ...
            // {
            //     if(Monsters[i] == creature_id)
            //     {
            //         MonstersV.push_back(Visible_creature(creature_id, creature_x, creature_y, creature_vx, creature_vy, color, type));
            //     }
            // }
        }
        ...
        // my_drone[1].Monsters.push_back(Visible_creature(0, foe_drone_x, foe_drone_y, 0, 0, 0, 0));
        ...
        // for(size_t i=0; i<OldMonstersDrone1.size(); i++)
        //     my_drone[0].Monsters.push_back(OldMonstersDrone1[i]);
        // for(size_t i=0; i<OldMonstersDrone2.size(); i++)
        //     my_drone[1].Monsters.push_back(OldMonstersDrone2[i]);
        
        // OldMonstersDrone1.clear();
        // OldMonstersDrone2.clear();
        cerr << "size1 = " << my_drone[0].Monsters.size() << endl;;
        cerr << "size2 = " << my_drone[1].Monsters.size() << endl;

        // cerr << "size1 = " << OldMonstersDrone1.size() << endl;
        // cerr << "size2 = " << OldMonstersDrone2.size() << endl;

        for (int i = 0; i < my_drone_count; i++)
        {
            // if(Turn % 4 == 1 && my_drone[i].drone_y > 2500)
            //     my_drone[i].light = 1;
            if(my_drone[i].drone_y >= 5000)
                my_drone[i].light = 1;
            if(i == 0)
        ...
        // OldMonstersDrone1 = my_drone[0].Monsters;
        // OldMonstersDrone2 = my_drone[1].Monsters;
        // my_drone[0].Monsters.clear();
        // my_drone[1].Monsters.clear();
